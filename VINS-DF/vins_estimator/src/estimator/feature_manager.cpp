                        ...
                        double &depth, double &cov2)
{
    ...
    if (localPoint.z() > 0){
        ...
        depth = INIT_DEPTH;
        cov2 = FOCAL_LENGTH/1.5 *FOCAL_LENGTH/1.5;
    }
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[], bool MARGIN_OLD)
{
    for (auto &it_per_id : feature)
    {
        if (it_per_id.estimated_depth <= 0){//若深度值未初始化
            if(STEREO && it_per_id.feature_per_frame[0].is_stereo)
            {
                int imu_i = it_per_id.start_frame;
                Eigen::Matrix<double, 3, 4> leftPose;
                Eigen::Vector3d t0 = Ps[imu_i] + Rs[imu_i] * tic[0];//世界坐标系下的位姿
        ...
        if(segma <=0.0001){//深度滤波器开关
            ...
            {
                it_per_id.estimated_depth = INIT_DEPTH;
                it_per_id.estimated_cov = FOCAL_LENGTH/1.5 *FOCAL_LENGTH/1.5;
            }
        }
        else if(MARGIN_OLD){
            ...
            for(;imu_j<imu_i + it_per_id.used_num;imu_j++){//深度滤波
                ...
                if(temp_depth!=INIT_DEPTH&&temp_depth>it_per_id.estimated_depth/2){//当结果有效时进行深度融合
                    //cout<<"merging"<<endl;

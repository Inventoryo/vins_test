...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    if (sequence_cnt != cur_kf->sequence)//初始化w_t_vio，w_r_vio，t_drift，r_drift
    {
        sequence_cnt++;
        sequence_loop.push_back(0);
        w_t_vio = Eigen::Vector3d(0, 0, 0);
    ...
    vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;//转到相机坐标系坐标系
    ...
    {
        TicToc tmp_t;
        loop_index = detectLoop(cur_kf, cur_kf->index);//检测闭环,DBoW
        //std::cout<<"suspect loop found in "<<loop_index<<std::endl;
    }
    else
    {
        addKeyFrameIntoVoc(cur_kf);
    }
	if (loop_index != -1)//检测到闭环
	{
        //printf(" %d detect loop with %d \n", cur_kf->index, loop_index);
        KeyFrame* old_kf = getKeyFrame(loop_index);

        if (cur_kf->findConnection(old_kf))//判断帧间变换，以及是否真的闭环
        {
            ...
            w_P_cur = w_R_old * relative_t + w_P_old;//當前幀通过闭环帧映射到世界坐标系
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)//save image in pose graph for visualization prupose
    {
    ...
    QueryResults ret;
    TicToc t_query;
    db.query(keyframe->brief_descriptors, ret, 4, frame_index - 50);//詞袋检索
    //printf("query time: %f", t_query.toc());
    //cout << "Searching for Image " << frame_index << ". " << ret << endl;

    TicToc t_add;
    db.add(keyframe->brief_descriptors);//// 向BriefDatabase中增加记录（代表图像的特征描述子）
    ...
    if (ret.size() >= 1 &&ret[0].Score > 0.05)
        ...
        {
            //std::cout<<"find loop in frame_index = "<<frame_index<<std::endl;

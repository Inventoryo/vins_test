		           ...
		           vector<double> &_point_id, int _sequence)
{
	...
	sequence = _sequence;
	computeWindowBRIEFPoint();//原特征点的描述子提取window_keypoints,window_brief_descriptors
	computeBRIEFPoint();//新提取fast角点，并提描述子，存入keypoints,brief_descriptors,keypoints_norm
	if(!DEBUG_IMAGE)
		image.release();
}
...
void KeyFrame::computeBRIEFPoint()
{
	...
	if(1)
		cv::FAST(image, keypoints, fast_th, true);//提取fast角点
	else
	{
	...
	for (int i = 0; i < (int)keypoints.size(); i++)
	{
		...
		keypoints_norm.push_back(tmp_norm);//归一化点
	}
}
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	double relative_yaw;
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)//至少匹配25个特征点
	{
		status.clear();
	    PnPRANSAC(matched_2d_old_norm, matched_3d, status, PnP_T_old, PnP_R_old);//ransac去除误匹配
	    reduceVector(matched_2d_cur, status);
	    reduceVector(matched_2d_old, status);
	    reduceVector(matched_2d_cur_norm, status);
	    reduceVector(matched_2d_old_norm, status);
	            ...
	            */
	            if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	            {
	            	...
	            	*/
	    	    	...
	    	    	pub_match_img.publish(msg);//发布回环图
	            }
	        }
	    #endif
	}
    //std::cout<<"matched_2d_cur.size() = "<<matched_2d_cur.size()<<std::endl;
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)//依然有25个点以上
	{
        ...
        //std::cout<<"abs(relative_yaw) = "<<abs(relative_yaw)<<" ; relative_t.norm() = "<<relative_t.norm()<<std::endl;
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)//旋转角与距离
	    {

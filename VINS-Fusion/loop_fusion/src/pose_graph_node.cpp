...
void process()//读取图片，特征点，位姿信息，建立keyframe
{
    while (true)
    {
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_vio = n.subscribe("/vins_estimator/odometry", 2000, vio_callback);//vio,imu
    ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback);//图片
    ros::Subscriber sub_pose = n.subscribe("/vins_estimator/keyframe_pose", 2000, pose_callback);//关键帧位姿,imu
    ros::Subscriber sub_extrinsic = n.subscribe("/vins_estimator/extrinsic", 2000, extrinsic_callback);//相机外参tic0
    ros::Subscriber sub_point = n.subscribe("/vins_estimator/keyframe_point", 2000, point_callback);//特征点信息,世界坐标系点云

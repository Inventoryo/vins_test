...
ProjectionOneFrameTwoCamFactor::ProjectionOneFrameTwoCamFactor(const Eigen::Vector3d &_pts_i, const Eigen::Vector3d &_pts_j,
                                                               ...
                                                               const double _td_i, const double _td_j, const double reciprocal_cov) :
                                                               ...
                                                               td_i(_td_i), td_j(_td_j)
{
    ...
    velocity_j.z() = 0;
    sqrt_info = (FOCAL_LENGTH + segma * sqrt(reciprocal_cov))/1.5*Eigen::Matrix2d::Identity();
    //cout<<" sqrt_info"<<(FOCAL_LENGTH +segma * sqrt(reciprocal_cov))/1.5<<endl;
...
bool ProjectionOneFrameTwoCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d tic(parameters[0][0], parameters[0][1], parameters[0][2]);//左相机外参
    Eigen::Quaterniond qic(parameters[0][6], parameters[0][3], parameters[0][4], parameters[0][5]);

    Eigen::Vector3d tic2(parameters[1][0], parameters[1][1], parameters[1][2]);//右相机外参
